...
-- perform a left join on user id
SELECT
	first_name, 
            last_name, tweet_date, 
            IFNULL(retweet_count, 0) AS retweet_count,
            IFNULL(favorite_count, 0) AS favorite_count
FROM users 
LEFT JOIN tweet_info ON users.id=tweet_info.user_id;

-- perform a right join on user id
SELECT
	tweet_info.id, first_name, last_name, 
            tweet_date, retweet_count, favorite_count, user_id 
FROM users 
RIGHT JOIN tweet_info ON users.id=tweet_info.user_id;